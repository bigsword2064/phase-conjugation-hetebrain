"""
Forward MRE simulation on the heterogeneous MIDA head mesh.

Solves the linear (mixed displacement/pressure) harmonic wave equation with a
Gaussian point source and an absorbing boundary condition, using per-tissue
material properties plus the MRE-derived stiffness heterogeneity in WM/GM
(from cube_hete_mre.py). Writes displacement/stress fields to disk
and saves the raw solution vectors (brain_uh_mpi_*.npy, brain_ph_mpi_*.npy)
for reuse by the phase-conjugation step in mida_hete_deep_sec.py.
"""
...
bcs = [bcs_1,bcs_2,bcs_3,bcs_top_x,bcs_top_y,bcs_top_z]
...
# define a spatially variant load with phase
V_source_magnitude = fem.functionspace(domain, ("Lagrange", 1))

x_ = ufl.SpatialCoordinate(domain)
amp_source = Function(V_source_magnitude)
r_ = ufl.sqrt((x_[0] - x0) ** 2 + (x_[1] - y0) ** 2 + (x_[2] - z0) ** 2)
sigma_source = 2.0  # Controls the width of your source peak

def gaussian_source(x):
    r2 = (x[0] - x0) ** 2 + (x[1] - y0) ** 2 + (x[2] - z0) ** 2
    return 300.0 * np.exp(-r2 / (2 * sigma_source**2))


amp_source.interpolate(gaussian_source)
amp_source.x.scatter_forward()

# define outward spreading wave source
direction = ufl.as_vector([(x_[0] - x0) / r_, (x_[1] - y0) / r_, (x_[2] - z0) / r_])
b_force = amp_source * direction
...
#
F = omega ** 2 * rho * ufl.inner(u, v) * ufl.dx - (ufl.inner(sigma(u,p_tr),grad(v))) * ufl.dx - 1j*omega*ufl.inner(ufl.dot(M1,u),v) * ds(100000) + ufl.inner((ufl.div(u) + p_tr / kappa), p_test) * ufl.dx + ufl.inner(b_force,v)*ufl.dx
a = ufl.lhs(F)
L = ufl.rhs(F)
...
s = sigma(uh,ph) - 1.0 / 3.0 * ufl.tr(sigma(uh,ph)) * ufl.Identity(len(uh))
...
# write output
uh.name = "Displacement_n"
stresses.name = "Shear wave_n"
results_folder = SCRIPT_DIR / "results_hete_deep_0.96"
results_folder.mkdir(exist_ok=True, parents=True)

with io.VTXWriter(MPI.COMM_WORLD, str(results_folder / "brain_hete_freq{}_disp.bp".format(f0)), [uh], engine="BP4") as vtx:
    vtx.write(0.0)

stress_file = io.XDMFFile(domain.comm, str(results_folder / "brain_hete_freq{}_stress.xdmf".format(f0)), "w")
stress_file.write_mesh(domain)
stress_file.write_function(stresses, 0)
stress_file.close()

kappa_file = io.XDMFFile(domain.comm, str(results_folder / "brain_kappa.xdmf"), "w")
kappa_file.write_mesh(domain)
kappa_file.write_function(kappa, 0)
kappa_file.close()
...
strain_t_file = io.XDMFFile(domain.comm, str(results_folder / "brain_hete_freq{}_strain_components.xdmf".format(f0)), "w")
strain_t_file.write_mesh(domain)
strain_t_file.write_function(strain_components, 0)
strain_t_file.close()

stress_t_file = io.XDMFFile(domain.comm, str(results_folder / "brain_hete_freq{}_stress_components.xdmf".format(f0)), "w")
...
np.save(str(results_folder / "brain_uh_mpi_{}.npy".format(MPI.COMM_WORLD.rank)), uh.x.array)
np.save(str(results_folder / "brain_ph_mpi_{}.npy".format(MPI.COMM_WORLD.rank)), ph.x.array)
